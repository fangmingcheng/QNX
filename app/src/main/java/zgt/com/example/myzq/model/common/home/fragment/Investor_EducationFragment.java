...
import zgt.com.example.myzq.model.common.adapter.Investor_EducationAdapter;
 ...
 */
public class Investor_EducationFragment extends BaseFragment {
    ...
    private Investor_EducationAdapter adapter;
    private int currentpage=1,totalcount=0;

    public Investor_EducationFragment() {
    ...
    public int getLayoutId() {
        return R.layout.fragment_investor__education;
    }
...
//    瀑布流
        ...
        adapter = new Investor_EducationAdapter(getActivity(), list);
        recyclerview.setAdapter(adapter);
        setPullRefresher();
        //设置 Header 为 Material风格
        refreshLayout.setRefreshHeader(new MaterialHeader(getActivity()).setShowBezierWave(true));
//        //设置 Footer 为 球脉冲
        adapter.setOnItemClickListener(new Investor_EducationAdapter.OnItemClickListener() {
            @Override
            public void OnItemClick(View view) {
                int position = recyclerview.getChildAdapterPosition(view);
                startActivity(new Intent().setClass(getActivity(), TrainingBaseDetailActivity.class).putExtra("uuid",list.get(position).getUuid()).putExtra("status","3"));
            }
        });
    }
